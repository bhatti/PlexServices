...
class MethodPayLoadRequest {
	private static final Logger logger = Logger.getLogger(MethodPayLoadRequest.class);
	...
	@VisibleForTesting
	static MethodPayLoadRequest getMethodNameAndPayloads(Request request, String defaultMethod) {
		...
		if (text != null && text.length() > 0) {
			try {
				...
				if (startObject != -1 && endObject != -1 && colonPos != -1) {
					int methodStart = incrWhileSkipWhitespacesAndQuotes(text, startObject + 1);
					...
					String payload = payloadStart <= payloadEnd ? text.substring(payloadStart, payloadEnd + 1).trim()
							: "";
					MethodPayLoadInfo info = new MethodPayLoadInfo(method, payload);
		...
		if (methodPayLoadRequest.requests.size() == 0) {
			...
			//
			if (method != null) {
				MethodPayLoadInfo info = new MethodPayLoadInfo(method, text == null ? text : text.trim());
				methodPayLoadRequest.requests.add(info);
			}
		}
		//
		if (methodPayLoadRequest.requests.size() == 0) {
			throw new IllegalArgumentException("Could not find method-name in request " + request.getProperties());
		}
		return methodPayLoadRequest;
	}

	private static void parseMethodPayload(MethodPayLoadRequest methodPayLoadRequest, JsonNode node) {
		if (node.isObject()) {
			...
			if (it.hasNext()) {
				...
				MethodPayLoadInfo info = new MethodPayLoadInfo(methodName, payload);
				methodPayLoadRequest.requests.add(info);
			}
		}
	}
	...
	private static int incrWhileSkipWhitespacesAndQuotes(String text, int start) {
		while (Character.isWhitespace(text.charAt(start)) || text.charAt(start) == '\'' || text.charAt(start) == '"') {
			start++;
		}
		return start;
	}
	...
	private static int decrWhileSkipWhitespacesAndQuotes(String text, int end) {
		while (Character.isWhitespace(text.charAt(end)) || text.charAt(end) == '\'' || text.charAt(end) == '"') {
			end--;
		}
		return end;
	}
